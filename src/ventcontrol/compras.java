...
public class compras extends JDialog {
    ...
    Integer band=0;
    Integer usuarioactu;
    public compras(menu menuprincipal, boolean modal, Integer usuactu) {
    ...
    compras(menu2 mimenu2, boolean b) {
...
//        }else{
            sql="SELECT * FROM compra c inner join proveedor p on c.proveedor_codprov=p.codprov where fecha BETWEEN '"+fechaini.getDate()+"' and '"+fechafin.getDate()+"' ORDER BY codcompra";
...
//        }                
        ...
        try{
                ...
                tablacliente.getColumnModel().getColumn(2).setPreferredWidth(250);
    ...
    void cargarci(String valor){
        String [] titulos ={"Cod","Fecha","Proveedor","Nro. Factura", "Total"};
        ...
        if(valor.equals("")){
            ...
            sql="SELECT * FROM compra c inner join proveedor p on c.proveedor_codprov=p.codprov where fecha BETWEEN '"+fechaini.getDate()+"' and '"+fechafin.getDate()+"' and p.nombre LIKE '%"+valor+"%' ORDER BY codcompra";
            System.out.print("entra en el segundo");
        }                
        model = new DefaultTableModel (null, titulos){
        @Override
        ...
        try{
                ...
                while(rs.next()){
                    ...
                    st = cn.createStatement();
                    conta = conta+1;
                    monto = monto + Integer.parseInt(registros[4]);  
                    ResultSet as = st.executeQuery(sql1);
                    while(as.next()){
                        registros[2] = as.getString("nombre");                       
                    }
    ...
    private void initComponents() {
        ...
        jLabel5.setText("N° COMPRAS:");
        getContentPane().add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(600, 520, -1, 30));
        ...
        getContentPane().add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(790, 520, -1, 30));
...
//        m.show();
    ...
    private void viewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewActionPerformed
        ...
        cp = new updatecompra(mimenu, true, codigo1, codigo, fechaini.getDate(), fechafin.getDate(), usuarioactu);        
        cp.setVisible(true); 
        Integer conta=0, monto=0;        
        if(cp.modeloRefresca!=null){
            ...
            for(int z =0; z<tablacliente.getRowCount(); z++){                
                conta = conta+1;
                //monto = monto + Integer.parseInt(tablacliente.getValueAt(z, 4).toString()); 
                try{
                        Number kore = formateador.parse(tablacliente.getValueAt(z, 5).toString());
                        num1=kore.intValue();
                        monto = monto + num1;                       
    ...
    private void delete1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_delete1ActionPerformed
        ...
        if(FilaSelec>=0)            
        {
            ...
            try{        
                int confirmar = JOptionPane.showConfirmDialog(null, "Desea Eliminar la Compra?");
                    if(confirmar==JOptionPane.YES_OPTION){
                            String sql1="SELECT * FROM detcompra where compra_codcompra='"+cod+"'";
                            conectar cc = new conectar();
                            Connection cn = cc.conexion();
                            ...
                            while(rsaux.next()){
                                ...
                                st2.executeUpdate();
                                sql2 ="DELETE FROM detcompra where compra_codcompra='"+rsaux.getString("compra_codcompra")+"'";
                                PreparedStatement st1 = cn.prepareStatement(sql2);
                                st1.executeUpdate();
                            }                                
                            sql ="DELETE FROM compra where codcompra='"+cod+"'";                                                                 
