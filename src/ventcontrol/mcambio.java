 ...
 * @author Usuario
 */
public class mcambio extends JDialog {
    ...
    public mcambio(menu menuprincipal, boolean modal, Integer usuario) {
    ...
    void cargar(String valor){
        String [] titulos ={"Cod","Fecha","Producto Uno", "Producto Dos","Cantidad", "Usuario"};
...
//        }                
    ...
    private void initComponents() {
        ...
        getContentPane().add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 80, 740, 360));
        ...
        iconproveedor.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/produc.png"))); // NOI18N
        getContentPane().add(iconproveedor, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 290, 150, 150));

        jLabel2.setFont(new java.awt.Font("Khmer UI", 1, 18)); // NOI18N
        jLabel2.setForeground(new java.awt.Color(240, 240, 240));
        jLabel2.setText("CAMBIO DE MERCADERÍAS.");
        ...
        view1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/new.png"))); // NOI18N
        view1.setText("   Nuevo");
        ...
        getContentPane().add(view1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 80, 160, 60));
...
//        m.show();
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        ...
        if(FilaSelec>=0){
            ...
            try{        
                int confirmar = JOptionPane.showConfirmDialog(null, "Desea Eliminar el Cambio?");
                    if(confirmar==JOptionPane.YES_OPTION){
                            String sql3="SELECT * FROM detmerca where cmerca_id='"+cod+"'";
                            Statement staux = cn.createStatement();
                            ResultSet rsaux = staux.executeQuery(sql3);
                            String codprodu="";
                            while(rsaux.next()){
                                ...
                                while(rs1.next()){
                                    if(rsaux.getString("estado").equals("Donante")){
                                        stockaux= Double.parseDouble(rs1.getString("stock"));
                                        totalstock=stockaux+Double.parseDouble(rsaux.getString("cantidad"));
                                        codprodu = rs1.getString("codprodu");
                                    }else{
                                        stockaux= Double.parseDouble(rs1.getString("stock"));
                                        codprodu = rs1.getString("codprodu");
                                        if(stockaux>Integer.parseInt(rsaux.getString("cantidad"))){
                                            totalstock=stockaux-Integer.parseInt(rsaux.getString("cantidad"));                                            
                                        }else{
                                            JOptionPane.showMessageDialog(null, "El producto ya se Vendio.");
                                            this.dispose();
                                        }                                        
                                    }
                                }
                                ...
                                PreparedStatement st5 = cn.prepareStatement(sql4);
                                st5.executeUpdate();
                                String sql7 ="DELETE FROM detmerca where id='"+rsaux.getString("id")+"'";
                                PreparedStatement st7 = cn.prepareStatement(sql7);
                                st7.executeUpdate();
                            }
                            sql ="DELETE FROM cmerca where id='"+cod+"'";                                    
                            //conectar cc = new conectar();
                            //Connection cn = cc.conexion(); 
                            ...
                            if(st.executeUpdate()>0){
                                JOptionPane.showMessageDialog(null, "Se eliminó correctamente el Cambio.");                      
                                st.close(); 
                            }  
    ...
    private void view1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_view1ActionPerformed
        ...
        caja = new cargarmcambio(mimenu, true, usuarioactu, "");
        caja.setVisible(true);
        if(caja.model!=null){
            System.out.print("       Aca ni de bromaaa    ");
            tablacliente.setModel(caja.model);  
            tablacliente.getColumnModel().getColumn(0).setPreferredWidth(40);
            tablacliente.getColumnModel().getColumn(1).setPreferredWidth(60);
            tablacliente.getColumnModel().getColumn(2).setPreferredWidth(200);
            tablacliente.getColumnModel().getColumn(3).setPreferredWidth(200);
