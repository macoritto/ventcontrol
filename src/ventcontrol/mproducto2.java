 ...
 * @author Usuario
 */
public class mproducto2 extends JDialog {

    DefaultTableModel model;
    DefaultComboBoxModel modeloRol;
    Integer band=0;
    public mproducto2(menu menuprincipal, boolean modal) {
    ...
    mproducto2(menu2 mimenu2, boolean b) {
    ...
    void cargarprodu(String valor){
        ...
        String sql, sql1;        
        sql="SELECT * FROM producto where codprodu='"+valor+"' and coprodu!='10000' ORDER BY codprodu";
...
//        }                
    ...
    void cargar(String valor){
        String [] titulos ={"Cant. Vendida","Descripcion","Cod","P. Venta", "Subtotal"};
        ...
        Double contador2=0.0, acum2=0.0;    
        try{
                conectar cc = new conectar();
                Connection cn = cc.conexion(); 
                Statement st = cn.createStatement();
                ...
                DecimalFormat formateador = new DecimalFormat("###,###");
                while(rs.next()){
                    String sql2="SELECT * FROM detventa where producto_codprodu='"+rs.getString("codprodu")+"'";                    
                    //registros[2] = formateador.format(Integer.parseInt(rs.getString("costo")));                                                                                
                    st = cn.createStatement();
                    ResultSet as = st.executeQuery(sql2);
                    Double contador=0.0, acum=0.0;                                        
                    while(as.next()){
                        String sql3="SELECT * FROM venta where codventa='"+as.getString("venta_codventa")+"' and fecha BETWEEN '"+fechaini.getDate()+"' and '"+fechafin.getDate()+"'";
                        st = cn.createStatement();
                        ResultSet as1 = st.executeQuery(sql3);
                        while(as1.next()){                                                        
                            contador= contador+Double.parseDouble(as.getString("cantidad"));
                            acum = acum+Integer.parseInt(as.getString("total"));  
                        }
                    } 
                    if(contador>0){
                        registros[2] = rs.getString("codprodu");
                        registros[1] = rs.getString("nomprodu");
                        registros[3] = formateador.format(Integer.parseInt(rs.getString("venta")));
                ...
                rs.close();
                cargarventas("");
                nrocompras.setText(formateador.format(contador2));                                
                RowSorter<TableModel> sorter = new TableRowSorter<TableModel>(model);                
                tablacliente.setModel(model);   
                tablacliente.setRowSorter(sorter);
                tablacliente.getRowSorter().toggleSortOrder(0); 
                tablacliente.getRowSorter().toggleSortOrder(0); 
...
void cargarprov(String valor){
    ...
    private void initComponents() {
        ...
        getContentPane().add(search, new org.netbeans.lib.awtextra.AbsoluteConstraints(610, 20, 40, 40));
        ...
        getContentPane().add(fechaini, new org.netbeans.lib.awtextra.AbsoluteConstraints(660, 30, 170, 30));
        ...
        jLabel6.setText("MEJORES PRODUCTOS.");
        ...
        getContentPane().add(jLabel8, new org.netbeans.lib.awtextra.AbsoluteConstraints(850, 10, -1, -1));
        getContentPane().add(fechafin, new org.netbeans.lib.awtextra.AbsoluteConstraints(850, 30, 170, 30));

        jLabel9.setFont(new java.awt.Font("Khmer UI", 1, 14)); // NOI18N
        jLabel9.setForeground(new java.awt.Color(240, 240, 240));
        jLabel9.setText("FECHA INICIO:");
        getContentPane().add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(660, 10, -1, -1));
