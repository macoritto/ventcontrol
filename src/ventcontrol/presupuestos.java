 ...
 * @author Usuario
 */
public class presupuestos extends JDialog {

    DefaultTableModel model;
    DefaultComboBoxModel modeloRol;
    Integer band=0;
    Integer usuarioactu;
    public presupuestos(menu menuprincipal, boolean modal, Integer usuactu) {
    ...
    presupuestos(menu2 mimenu2, boolean b) {
...
//        }else{
            sql="SELECT * FROM presupuesto c inner join cliente p on c.cliente_id=p.id where fecha BETWEEN '"+fechaini.getDate()+"' and '"+fechafin.getDate()+"' ORDER BY codpresu";
...
//        }                
        ...
        try{
                ...
                while(rs.next()){                    
                    registros[0] = rs.getString("codpresu");
    ...
    void cargarci(String valor){
        ...
        String sql, sql1;
        if(valor.equals("")){
            sql="SELECT * FROM presupuesto ORDER BY codpresu";
            System.out.print("entra en el simple");
        }else{
            sql="SELECT * FROM presupuesto c inner join cliente p on c.cliente_id=p.id where fecha BETWEEN '"+fechaini.getDate()+"' and '"+fechafin.getDate()+"' and p.nombre LIKE '%"+valor+"%' ORDER BY codpresu";
            System.out.print("entra en el segundo");
        }                
        model = new DefaultTableModel (null, titulos){
        @Override
...
//        m.show();
    ...
    private void viewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewActionPerformed
        ...
        updatepresupuesto cp;
        menu mimenu;
        mimenu = new menu(0);
        this.band=2;
        System.out.print(codigo);
        System.out.print("valor de la tabla");
        cp = new updatepresupuesto(mimenu, true, codigo1, codigo, fechaini.getDate(), fechafin.getDate(), usuarioactu);        
        cp.setVisible(true); 
        Integer conta=0, monto=0;        
...
//            if(contador==0){
            ...
            try{        
                int confirmar = JOptionPane.showConfirmDialog(null, "Desea Eliminar el Presupuesto?");
                    if(confirmar==JOptionPane.YES_OPTION){
//                            String sql1="SELECT * FROM detpresu where presupuesto_codpresu='"+cod+"'";
                            conectar cc = new conectar();
                            Connection cn = cc.conexion();
...
//                                st2.executeUpdate();
                                sql2 ="DELETE FROM detpresu where presupuesto_codpresu='"+cod+"'";
                                PreparedStatement st1 = cn.prepareStatement(sql2);
                                st1.executeUpdate();
                            //}                                
                            sql ="DELETE FROM presupuesto where codpresu='"+cod+"'";                                                                 
