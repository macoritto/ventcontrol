 ...
 * @author Usuario
 */
public class retirocaja extends JDialog {

    DefaultTableModel model;
    DefaultComboBoxModel modeloRol;
    Integer band=0;
    Integer sumar=0, sumav=0, sumai=0, sumap=0, sumad=0;
    Integer usuarioactu, codigogasto;
    public retirocaja(menu menuprincipal, boolean modal, Integer usuario) {
    ...
    retirocaja(menu2 mimenu2, boolean b) {
    ...
    void cargarpagos(String valor){
        ...
        try{
                ...
                while(rs.next()){
                    ...
                    sumap = sumap + Integer.parseInt(rs.getString("monto"));
    ...
    void cargardevo(String valor){
        ...
        try{
                ...
                while(rs.next()){
                    ...
                    registros[3] = rs.getString("descripcion");                 
                    model.addRow(registros);   
                    sumad = sumad + Integer.parseInt(rs.getString("total"));
                    //JTableHeader header = tablausu.getTableHeader();
    ...
    private void autonumerargasto(){
            String sql="SELECT coalesce (max(id+1),1) as newid from gastos";
            conectar cc = new conectar();
            Connection cn = cc.conexion(); 
        try
        {
            ...
            ResultSet rs = st.executeQuery(sql);       
            rs.next();
            codigogasto=Integer.parseInt(rs.getString("newid"));
            //cod.setText(rs.getString("newid"));
        cn.close();
        }catch(SQLException ex){
...
//        m.show();
    ...
    private void guardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarActionPerformed
        ...
        menu mimenu;
        mimenu = new menu(0);
        caja = new listacaja(mimenu, true);
    ...
    private void guardar1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardar1ActionPerformed
        ...
        totalingresos=sumav+sumai+sumap;
        diferencia=totalingresos-sumar-sumad;
        aux3 = monto.getText();
        Number w=0;
        try {
            Integer a =1;
            ...
            Logger.getLogger(retirocaja.class.getName()).log(Level.SEVERE, null, ex);
        }
        monto5 = w.intValue();
        if(monto5>0){
                ...
                if(monto.getText().length()!=0){
                    if(diferencia>=monto5){
                            try{                                  
                                        ...
                                        String sql ="INSERT INTO retiro (fecha, hora, monto, detalle, cod, usuario, caja) VALUES ('" +calendar.getDate()+ "','"+aux+"','" +monto4.toString()+"','" +descrip.getText()+ "','"+cod.getText()+"','"+usuarioactu+"','1')";                                                        
                                        ...
                                        if(st.executeUpdate()>0){
                                            int confirmar = JOptionPane.showConfirmDialog(null, "Desea Registrar como gasto el Retiro?");
                                            if(confirmar==JOptionPane.YES_OPTION){
                                                autonumerargasto();
                                                //monto.setText(monto4.toString());
                                                String sql2 ="INSERT INTO gastos (id, fecha, monto, descripcion, usuario, caja, idaux) VALUES ('" +codigogasto+"','"+calendar.getDate()+"','" +monto4.toString()+"','" +descrip.getText()+ "','"+usuarioactu+"','1','"+cod.getText()+"')";                                                        
                                                ...
                                                if(st1.executeUpdate()>0){
                                                        //cargar("");
                                                        JOptionPane.showMessageDialog(null, "Se Registro el retiro.");                               
                                                }                    
                                                //p.cargar(valor);                          
                                                st.close();                        
                                                this.dispose();
                                                ...
                                                JOptionPane.showMessageDialog(null, "Se realizo el retiro.");
                                            }
                                        }                    
                                        //p.cargar(valor);                          
                                        st.close();                        
